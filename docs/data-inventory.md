# Labfluss Data Inventory and Classification
...
This document inventories the principal categories of data stored or processed by Labfluss and assigns an internal data classification.
...
This inventory should be reviewed whenever Labfluss begins storing a materially new category of customer, authentication, research, or operational data.

## Classification Model

Labfluss uses four internal classifications.
...
Customer-entered Labfluss data should not normally be classified as Public unless the customer has intentionally made it public.
...
Most normal Labfluss customer content falls into this category.
...
Information whose disclosure or misuse could enable unauthorized access, compromise accounts or infrastructure, or materially weaken Labfluss security.
...
| Attachment file contents          | research documents, spreadsheets, images, presentations and other supported research files                                    | private Amazon S3                                                                       | Confidential                                                             | authorized users through short-lived signed access                                                  |
| Archived application records      | archived projects, tasks, experiments, protocols and attachments                                                              | PostgreSQL and Amazon S3 where applicable                                               | Same classification as active record                                     | admins and authorized recovery workflows                                                            |
| Transactional email content       | invitation, verification and password-reset email addresses and message content                                               | Amazon SES and delivery path                                                            | Confidential; embedded active token links Security-Sensitive while valid | intended recipient, Labfluss backend and provider as required                                       |
...
| Application logs                  | request identifiers, operational events, sanitized errors, cleanup events                                                     | AWS Lightsail systemd journal / application logging environment                         | Internal to Confidential                                                 | Labfluss operator                                                                                   |
| Monitoring data                   | uptime checks, service health/readiness status, alert history                                                                 | Better Stack                                                                            | Internal                                                                 | Labfluss operator                                                                                   |
| Database backups                  | copy of relational application data                                                                                           | backup locations                                                                        | Security-Sensitive                                                       | Labfluss operator/recovery process                                                                  |
| Attachment backups                | copies of customer attachment objects                                                                                         | backup locations                                                                        | Confidential                                                             | Labfluss operator/recovery process                                                                  |
| Production configuration metadata | service names, non-secret environment configuration, deployment settings                                                      | AWS Amplify, AWS Lightsail, Amazon RDS, Amazon S3, Route 53, and provider configuration | Internal                                                                 | Labfluss operator                                                                                   |
| Production secrets                | JWT_SECRET, SES secret access key, AWS access-key credentials, database-secret access credentials, and comparable credentials | hosting/provider secret configuration                                                   | Security-Sensitive                                                       | Labfluss operator and required backend runtime                                                      |

## Primary Data Processors and Storage Providers

The current production deployment uses:

- Amazon RDS for PostgreSQL for relational application data
- Amazon S3 for private attachment objects
...
Not every provider stores customer research content. The specific data categories processed by each provider are identified throughout this inventory and should be carried forward into the Labfluss subprocessor inventory.

## System-Specific Notes

### PostgreSQL

PostgreSQL is Labfluss's primary relational data store.
...
The production PostgreSQL database is hosted on Amazon RDS in the Europe (Frankfurt) Region and is private-only. The backend reaches it through private Lightsail-to-RDS connectivity.

### Amazon S3

Amazon S3 stores the binary content of Labfluss attachments.

The bucket is private.

Attachment objects inherit the classification of their contents and should normally be treated as Confidential.

Labfluss does not assume that an attachment is harmless based solely on its file type.

Access to attachments is controlled by the associated Labfluss resource permissions and short-lived signed URLs.

### Browser-Side Data

The Labfluss frontend stores an authentication token to support persistent login.
...
Backup access should be limited to the Labfluss operator and recovery processes.

## Customer Data Versus Labfluss Operational Data

### Customer Data

Customer Data includes information entered, created, uploaded, or generated through customer use of Labfluss, including:
...
Data prohibited by the Labfluss Pilot Data Policy remains prohibited.
...
Classification does not replace Labfluss's authorization model.
...
- when a new external service begins processing Labfluss data
