...
| Production configuration metadata | service names, non-secret environment configuration, deployment settings                         | AWS Amplify, AWS Lightsail, Amazon RDS, Route 53, Cloudflare R2, and provider configuration | Internal                                                                 | Labfluss operator                                                                                   |
| Production secrets                | DATABASE_URL, JWT_SECRET, Mailgun API key, R2 access keys/secrets and comparable credentials     | hosting/provider secret configuration                                                       | Security-Sensitive                                                       | Labfluss operator and required backend runtime                                                      |
...
Cloudflare R2 stores the binary content of Labfluss attachments.
